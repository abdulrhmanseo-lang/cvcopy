...
const Auth: React.FC = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            await login(email);
            showToast(`مرحباً بك ${isLogin ? 'مجدداً' : ''}`, 'success');
        ...
        <div className="min-h-screen bg-dark flex items-center justify-center p-4 relative overflow-hidden">
            ...
            <div className="bg-card/50 backdrop-blur-xl border border-white/10 w-full max-w-md p-8 rounded-3xl shadow-2xl relative z-10 animate-fade-up">
                ...
                <form className="space-y-4" onSubmit={handleSubmit}>
                    {!isLogin && (
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="w-full bg-dark border border-white/10 rounded-xl py-3 pr-12 pl-4 text-white focus:border-primary focus:ring-1 focus:ring-primary outline-none transition-all"
                            />
                        </div>
                    )}
                    <div className="relative">
                    ...
                    <button disabled={isLoading} className="w-full bg-primary hover:bg-primary-dark text-white font-bold py-3.5 rounded-xl shadow-[0_0_20px_rgba(124,58,237,0.3)] transition-all active:scale-95 mt-4 flex items-center justify-center gap-2 disabled:opacity-70">
